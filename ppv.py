"""Python wrapper around the Swift+Metal squaring kernel.

The heavy lifting (the squaring loop, dense GEMM on the GPU via
MPSMatrixMultiplication, vector reductions) runs in
``swift_backend/libppv_metal.dylib``. This module is a thin ctypes shim that
keeps the public ``compute(...)`` signature unchanged.

Build the dylib once with ``make -C swift_backend`` before running any tests.
"""
...
if not os.path.exists(_DLL_PATH):
    raise ImportError(
        f"Metal kernel not found at {_DLL_PATH}. "
        f"Build it with `make -C swift_backend`."
    )

_lib = ctypes.cdll.LoadLibrary(_DLL_PATH)
_lib.ppv_metal_squaring.argtypes = [
    ctypes.POINTER(ctypes.c_float),    # Q
    ctypes.POINTER(ctypes.c_float),    # R
    ctypes.c_int32,                     # ndi
    ctypes.c_int32,                     # num_delegates
    ctypes.c_int32,                     # num_policies
    ctypes.c_float,                     # tol
    ctypes.c_int32,                     # max_iter
    ctypes.POINTER(ctypes.c_float),    # consensus_out
    ctypes.POINTER(ctypes.c_float),    # influence_out
]
_lib.ppv_metal_squaring.restype = ctypes.c_int32
...
def compute(
    ...
    """Propagational Proxy Voting via Swift+Metal squaring kernel."""
    ...
    consensus_arr = np.zeros(num_policies, dtype=np.float32)
    influence_arr = np.zeros(ndi, dtype=np.float32)

    iters = _lib.ppv_metal_squaring(
        q.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
        r.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
        ndi, num_delegates, num_policies,
        ctypes.c_float(tol), max_iter,
        consensus_arr.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
        influence_arr.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
    )
    if iters < 0:
        raise MemoryError("Metal kernel allocation failed")
    if iters >= max_iter:
        warnings.warn(
            f"Metal kernel did not converge within {max_iter} squarings",
            stacklevel=2,
        )

    log.debug("metal_squaring: ndi=%d iters=%d num_policies=%d",
...
def _build_blocks(
    ...
    """Build dense row-major fp32 Q (ndi x ndi) and R (num_policies x ndi) from dicts.

    Skips the full n x n matrix that earlier branches built — at n=20000 that
    intermediate alone was 1.6 GB in fp32. Now we go dict → Q + R, ~1.3 GB total.
    """
